...
import '../main.dart';
import 'auth_screen.dart'; // ✅ ADDED - fixes LoginScreen reference
...
class _AdminShellState extends State<AdminShell> {
  ...
  Widget _adminBody(BuildContext context, AppUser user) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          // Close button → go to user home instead of popping
          IconButton(
  ...
  void _goToUserHome() {
    // Navigate to regular user home without losing admin session
  ...
  Future<void> _signOutAndGoToLogin() async {
    ...
    if (mounted) {
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => LoginScreen()), // ✅ const removed
      );
    }
  }
}
      ...
      GridView.count(
        ...
        children: [
          ...
          StreamBuilder<List<Product>>(
              ...
              builder: (_, s) => _StatCard('Products', '${s.data?.length ?? 0}',
                  Icons.inventory_2_rounded, AppColors.primary)),
          StreamBuilder<List<Shop>>(
              stream: db.allShopsStream(),
              builder: (_, s) => _StatCard('Shops', '${s.data?.length ?? 0}',
                  Icons.store_rounded, AppColors.amber)),
          StreamBuilder<List<PriceReport>>(
              stream: db.allReportsStreamFull(),
              builder: (_, s) => _StatCard('Reports', '${s.data?.length ?? 0}',
          ...
          StreamBuilder<List<AppUser>>(
              ...
              builder: (_, s) {
                ...
                return _StatCard('Banned Users', '$banned', Icons.block_rounded,
                    AppColors.accent);
              }),
        ],
      ),
            ...
            children: reports.isEmpty
                ...
                : reports
                    ...
                    .map((e) => Column(children: [
                          _ReportTile(
                              report: e.value,
                              onDelete: () => db.deleteReport(e.value.id)),
...
class _StatCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
            ...
            border: Border.all(
                color: color.withOpacity(urgent ? 0.4 : 0.2), width: 0.5)),
        ...
        StreamBuilder<List<AdminRequest>>(
          ...
          builder: (_, snap) {
            ...
            return ListView.separated(
              ...
              itemBuilder: (_, i) =>
                  _RequestCard(request: reqs[i], reviewerUid: uid, db: db),
            );
          },
        ),
        // All
...
class _RequestCard extends StatelessWidget {
    ...
    return Card(
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 12),
          _InfoRow(
              Icons.business_outlined, 'Organization', request.organization),
  ...
  Future<void> _reject(BuildContext context) async {
    ...
    if (ok == true && context.mounted) {
      await db.rejectAdminRequest(
          request.uid, reviewerUid, reasonCtrl.text.trim());
      if (context.mounted)
...
class _UserTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      subtitle: Text(
          '${user.email} · ${user.reportCount} reports · ${user.points} pts',
          style: const TextStyle(fontSize: 11, color: AppColors.gray400),
          overflow: TextOverflow.ellipsis),
      trailing: PopupMenuButton<String>(
        ...
        itemBuilder: (_) => [
          ...
          _menuItem(
              user.isAdmin ? 'remove_admin' : 'make_admin',
  ...
  void _handleAction(BuildContext ctx, String action) async {
    switch (action) {
      ...
      case 'make_admin':
        ...
        _snack(ctx, '${user.displayName} is now admin', true);
        break;
      case 'remove_admin':
        await db.setUserRole(user.uid, 'user');
        _snack(ctx, 'Admin role removed', true);
        break;
      case 'ban':
        await db.banUser(user.uid, true);
        _snack(ctx, '${user.displayName} banned', false);
        break;
      case 'unban':
        await db.banUser(user.uid, false);
        _snack(ctx, '${user.displayName} unbanned', true);
        break;
    }
  }
          ...
          if (_q.isNotEmpty)
            products = products
                .where((p) =>
                    ...
                    p.category.toLowerCase().contains(_q.toLowerCase()))
                .toList();
          ...
          return ListView.separated(
            ...
            itemBuilder: (_, i) {
              ...
              return ListTile(
                ...
                subtitle: Text(
                    '${p.category} · ${p.unit} · ETB ${p.avgPrice.toStringAsFixed(0)} avg · ${p.reportCount} reports',
        ...
        return ListView.separated(
          ...
          itemBuilder: (_, i) {
            ...
            return ListTile(
              ...
              subtitle: Text(
                  '${s.area} · ${s.reportCount} reports · ⭐ ${s.rating.toStringAsFixed(1)}',
                  style: const TextStyle(fontSize: 11)),
              trailing: PopupMenuButton<String>(
                icon: const Icon(Icons.more_vert_rounded,
                    size: 18, color: AppColors.gray400),
      ...
      builder: (ctx, snap) {
        ...
        return ListView.separated(
          ...
          itemBuilder: (_, i) => _ReportTile(
              report: reports[i],
              onDelete: () => db.deleteReport(reports[i].id)),
        );
      },
    );
  }
...
class _ReportTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      title: Row(children: [
        Expanded(
            child: Text('${report.productName} @ ${report.shopName}',
                style:
      ...
      subtitle: Row(children: [
        ...
        Container(
            ...
            decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(8)),
            child: Text(typeLabels[report.type] ?? '',
                style: TextStyle(
